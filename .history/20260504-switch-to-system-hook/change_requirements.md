...
| 課題ID | 課題 | 影響 | 解決方針 |
| --- | --- | --- | --- |
| CHG-SH-01 | Group Webhook を前提にした運用が GitLab CE では成立しない | GitLab CE 環境でリアルタイム連携方式を標準手順として案内できない | Group Webhook を正式に廃止し、System Hook を標準方式に置き換える |
...
- System Hook で有効化するイベントは Issue 更新イベントと Merge Request 更新イベントのみとする
...
| No. | 業務名 | 区分 | 変更内容 |
| --- | --- | --- | --- |
| B-1 | Issue／MR 検出運用 | 変更 | Group Webhook 前提を廃止し、System Hook を標準方式に変更する |
...
flowchart TD
    ...
    E --> J[URLとSecret Tokenと対象イベントを入力して保存]
    H --> K[導入手順書の接続確認手順で疎通確認]
    I --> K
    J --> K
    K --> L[Issue更新またはMR更新イベントを本システムが受信]
```
...
| 本システム | 既存 Webhook 受信口で System Hook イベントを受信し、Issue／MR 検出処理に引き渡す |
...
| 課題ID | 対応方針 | 機能区分 |
| --- | --- | --- |
| CHG-SH-01 | Group Webhook を削除し、System Hook を標準連携方式として扱う | 変更 |
...
| --- | --- | --- | --- | --- |
| F-SH-01 | System Hook 標準化 | 変更 | CHG-SH-01, CHG-SH-02 | Group Webhook 標準化を廃止し、System Hook を標準方式として扱う |
...
| データ | 出力先 | 説明 |
| --- | --- | --- |
| 登録済み System Hook 設定 | GitLab | 既存 URL と対象イベントを持つ System Hook |
...
| GitLab から本システム | System Hook | Issue 更新イベントと Merge Request 更新イベントの通知 |
...
flowchart TD
    ...
    I --> J[既存受信URLとSecret Tokenと対象イベントを入力]
    J --> K[保存]
    G --> L[接続確認]
    H --> L
    K --> L
    L --> M[Issue更新またはMR更新で受信確認]
```
...
| --- | --- | --- | --- | --- |
| TS-SH-01 | テスト環境セットアップ時に System Hook を自動登録できることを確認する | 環境変数設定に管理者用 PAT が設定されていない（自動生成される） | テスト環境セットアップを実行する | 管理者用 PAT が自動生成され、System Hook が登録または再利用され、セットアップが継続する |
| TS-SH-02 | 手動登録コマンドが冪等であることを確認する | 環境変数設定に管理者用 PAT が設定され、同一条件の System Hook が未登録である | 手動登録コマンドを 2 回実行する | 1 回目は登録、2 回目は再利用となり重複登録されない |
| TS-SH-03 | GUI 手順で System Hook を設定できることを確認する | GitLab 管理者が管理画面にログイン済みである | 導入手順書に従い URL、Secret Token、Issue 更新イベント、Merge Request 更新イベントを設定して保存する | System Hook が保存される |
| TS-SH-04 | 接続確認手順が成立することを確認する | System Hook が登録済みである | Issue 更新または MR 更新を発生させる | 本システムが既存受信口でイベントを受信できる |
| TS-SH-05 | 管理者用 PAT 未設定時の失敗導線を確認する（手動登録コマンドのみ対象） | 環境変数設定に管理者用 PAT が設定されていない | 手動登録コマンドを実行する | 管理者用 PAT が未設定であることが分かる失敗結果になる |
| TS-SH-06 | GUI 設定失敗時の確認観点が導入手順書で不足なく案内されることを確認する | 導入手順書が更新済みである | GUI 手順の失敗時確認観点を参照する | URL、Secret Token、対象イベント、管理者権限の確認観点が記載されている |
...
| カテゴリ | 該当要件 |
| --- | --- |
| 業務機能 | F-SH-01, F-SH-03, F-SH-04 |
| マスタ管理 | F-SH-02 |
| 共通 | F-SH-01 |
| 運用 | F-SH-05, F-SH-06 |
| 外部連携 | F-SH-01, F-SH-03, F-SH-04 |

### 7.2 機能とユーザーフローの対応

System Hook 標準化、管理者用 PAT 設定、手動登録、自動登録、導入手順更新、Group Webhook 削除のすべてが業務フローおよびユーザー利用フローに対応していることを確認した。

### 7.3 削除可能な要件

作成した要件を確認した結果、削除しても業務が成立する要件は残っていない。すべて GitLab CE 対応と設定漏れ防止の目的達成に必要である。

### 7.4 MVP 確認

本変更要件は以下の最小範囲に限定されている。

- Group Webhook 廃止と System Hook 標準化
- 管理者用 PAT の追加
- 手動登録とテスト環境自動登録
- 導入手順書の CUI / GUI 設定手順、接続確認、失敗時確認観点の追加
...
- Group Webhook 廃止と System Hook 標準化の方針に矛盾がないこと
- bot 用 PAT と管理者用 PAT の責務分離が一貫していること
- テスト環境セットアップ時の PAT 自動生成と、手動登録コマンド時の PAT 必須の差異が 1.3 前提・3.5.1・3.5.3 の三箇所で一貫して記述されていること
- CUI 自動登録と手動登録、GUI 手順の三系統が同一の受信 URL、Secret Token、対象イベントに収束していること
- TS-SH-01 がテスト環境での PAT 自動生成シナリオを包含し、TS-SH-05 が手動登録コマンドのみを対象としていること（テスト環境セットアップでは PAT 未設定でも成立するため）
- 導入手順追加要件が本番向け説明とテスト環境向け説明の両方を含んでいること
- 監視・アラートや専用ログなど、今回不要と確定した要件を含めていないこと

レビューの結果、冗長な要件や相互矛盾はないと判断した。
