...
| コンポーネント | 変更対象外の理由 |
|---|---|
| `producer/gitlab_event_handler.py` | System Hook の MR イベントペイロードは Group Webhook と同じ `object_kind: "merge_request"` フィールドを持つため、イベント振り分けロジックに変更不要。Issue 検出はポーリング方式を継続するため、Issue イベントの Webhook 受信処理は不要 |
...
| GitLab CE | `WebhookServer` | HTTP POST | `X-Gitlab-Event: System Hook` ヘッダーと MR イベントペイロード |
...
- **主な処理の流れ**:
  ...
  3. 一覧の中から `url`、`merge_requests_events` が全て一致する設定を検索する
...
- **`_register_system_hook()` の主な処理の流れ**:
  ...
  2. `url`・`merge_requests_events` の二条件で照合する
...
| M-SH-05 | GitLab CE | `WebhookServer` | MR 更新通知 | `POST /webhook` + `X-Gitlab-Event: System Hook` + ペイロード |
