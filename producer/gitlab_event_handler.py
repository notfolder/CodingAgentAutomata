...
class GitLabEventHandler:
    ...
    def __init__(
        ...
        # 処理済み Idempotency-Key を上限付き OrderedDict で管理する（重複受信抑止）
        # 最大 _MAX_PROCESSED_KEYS 件まで保持し、超過した場合は最古のキーを削除する
        self._processed_keys: OrderedDict[str, None] = OrderedDict()

    # Idempotency-Key の最大保持件数
    _MAX_PROCESSED_KEYS: int = 10000
    ...
    def handle_event(self, payload: dict, idempotency_key: Optional[str] = None) -> None:
        ...
        if idempotency_key is not None:
            if idempotency_key in self._processed_keys:
                ...
                return
            # 新規のキーを OrderedDict に追加して処理を継続する
            self._processed_keys[idempotency_key] = None
            # 最大保持件数を超過した場合は最古のキーを削除する（FIFO）
            if len(self._processed_keys) > self._MAX_PROCESSED_KEYS:
                self._processed_keys.popitem(last=False)
            logger.debug(
