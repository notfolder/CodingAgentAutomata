...
`docker compose up -d` 実行後に **初回のみ** 以下のコマンドを実行してください。
...
`docker compose --profile test up -d` およびシステムセットアップ後に実行してください。

```bash
# 必要な環境変数を設定
export GITLAB_ADMIN_TOKEN="<GitLab管理者PAT>"
export LITELLM_MASTER_KEY="<LiteLLMマスターキー>"
export WEBHOOK_URL="http://producer:8080/webhook"
