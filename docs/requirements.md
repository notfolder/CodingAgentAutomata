...
デフォルトテンプレートでは、F-3はIssueの内容からブランチ名とMRタイトルの生成のみを行う最小限の指示とし、F-4はMR descriptionを指示の本体としてそのまま渡す構成とする。F-3のデフォルトテンプレートは、CLIに対してブランチ名とMRタイトルを本システムが指定するJSON形式のファイルに出力するよう指示する構造とし、Consumerはcli-exec処理完了後に当該ファイルを読み取ってブランチ名とMRタイトルを取得する。
...
| GitLab PAT | 環境変数のみで管理し、DBには保存しない。cli-execコンテナには環境変数として渡さない。F-4（MR処理）のみgit clone URLにPATを埋め込んだ形式で使用する。F-3ではgit cloneは行わない。コンテナ破棄後にPATが残存しない |
...
上記項目を `.env` に設定した状態で `docker-compose up` を実行すると、GitLab・LiteLLM Proxy・本システム全コンポーネントが起動し、§6 のe2eテストシナリオを実施できる状態になることを設計上の制約とする。GitLab の初期設定（botアカウント作成・PAT発行・Webhook設定・テストプロジェクト作成）は `docker-compose up` 後に実行するセットアップスクリプトで自動化する。
...
| T-14 | タスク実行履歴を確認できる | T-07完了 | SC-07でステータスフィルタを「completed」に設定する | 完了タスクの一覧（タスクUUID・ユーザー名・CLI種別・完了日時）が表示される |
...
| T-23 | 一般ユーザーが自分のタスク実行履歴を閲覧できる | T-07完了、一般ユーザーでSC-01からログイン済み | SC-07（タスク実行履歴）を開く | 自分のタスクのみが表示される。ユーザー名フィルタは自分のみ固定で変更できない |
...
| T-25 | 管理者がSC-08でF-3用プロンプトテンプレートを変更できる | T-01完了（管理者でログイン済み） | SC-08（システム設定）を開き、F-3用テンプレートを編集して保存する | 保存成功のフラッシュメッセージが表示され、次回のF-3処理で変更後のテンプレートが使用される |
| T-26 | 管理者がSC-08でF-3用プロンプトテンプレートをリセットできる | T-25完了（テンプレートをカスタマイズ済み） | SC-08でF-3用テンプレートの「リセット」ボタンを押し確認ダイアログで実行する | テンプレートがシステムデフォルトに戻り、次回のF-3処理でデフォルトテンプレートが使用される |
| T-27 | 一般ユーザーがSC-08にアクセスできない | T-01完了（一般ユーザーでログイン済み） | /settings のURLへ直接アクセスする | 403エラーが返却される |
...
| T-29 | 管理者がSC-08で進捗コメント更新間隔と表示行数を変更できる | T-01完了（管理者でログイン済み） | SC-08の進捗報告設定で更新間隔を120秒・表示行数を30行に変更して保存する | 保存成功のフラッシュメッセージが表示され、次回のMR処理から120秒間隔・直近30行で進捗コメントが更新される |
