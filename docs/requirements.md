...
- GitLab連携（System Hook受信（MR）・ポーリング（Issue））: CUI（バックグラウンドプロセス）
- 初期管理者作成: CUI（コマンドライン）
...
flowchart TD
    ...
    B --> C{System Hook受信（MR）or ポーリング（Issue）検出}
    C --> C2{完了ラベルが付与済みか？}
...
| --- | --- | --- | --- |
| F-1 | System Hook受信 | P-1 | MR更新イベントをリアルタイムに検出できず MR 処理が遅延する |
| F-2 | ポーリング | P-1, P-3 | Issue の検出ができず、Issue→MR 変換処理を開始できない |
...
| F-10 | 重複処理防止 | P-1, P-3 | System Hook とポーリングがいずれも同一 MR を同時検出した場合に同一タスクが2重に実行される |
| F-11 | TTY入力待機検知 | P-1 | CLIが入力待機状態に陥った場合に処理が無限に停滞し、タスクが完了しない |
| F-12 | System Hook 標準化 | P-1, P-3 | GitLab CE では Group Webhook が利用できないため、MR 更新イベントの検出には GitLab CE でも動作するインスタンス全体向けの System Hook を標準方式とする。Issue 検出は既存のポーリング方式を継続する |
| F-13 | LLMキー・モデル設定検証 | P-2 | 無効なLLMキーが保存されてCLI実行時に認証エラーが発生し、タスクが失敗する |

### 3.2 入力データ

| データ | 種別 | 説明 |
| --- | --- | --- |
| System Hook ペイロード | 外部（GitLab） | MR更新イベントのJSONデータ。Issue検出はポーリングのみのため含まない |
...
| GitLab | System Hook（受信） | MR更新イベントのリアルタイム受信。Issue検出はポーリング方式を継続する |
...
ConsumerはCLI起動時にテンプレートから指示文を生成し、プロンプト本文は環境変数ではなく `/tmp/prompt.txt` を介してCLI実行コンテナへ受け渡す。
...
ConsumerはCLI実行中に一定間隔でGitLab MRの1件のコメントを作成または更新する。CLI終了時には完了またはエラーコメントを投稿する。F-4ではbotのアサイン解除を監視し、解除を検知した場合はCLIを強制終了する。

### 3.13 重複処理防止（F-10）

pending または running の同一タスクが重複しないよう、DB制約により排他制御する。

### 3.14 TTY入力待機検知（F-5拡張）

CLIが入力待機状態に陥った場合を自動検知し、タスクを強制終了してGitLabに失敗コメントを投稿する。

- Consumer（DinD実行前提）が eBPF + Tracee を使用して CLI コンテナの read(2) システムコールを監視する
- CLIが `/dev/pts/` または tty デバイスへの read 待機を検知した場合、CLI を強制終了して failed へ遷移する
- eBPF 利用可否（BTF 存在・CAP_BPF/CAP_PERFMON 権限）を起動時に判定し、不足時は TTY 検知を無効化してタスク処理を継続する
- TTY 検知時の失敗内容（error_message・cli_log）はタスク履歴に記録する

### 3.15 System Hook 運用（F-1 拡張・F-12）

GitLab System Hook を MR 更新イベント受信の標準方式とする。Issue 検出は既存のポーリング方式を継続する。

- GitLab 管理者がインスタンス全体に対して System Hook を一度設定することで、全プロジェクトの MR 更新イベントを受信する
- `GITLAB_ADMIN_PAT` 環境変数に管理者用 PAT を設定し、`python scripts/register_system_hook.py` で System Hook を冪等登録できる
- テスト環境セットアップ時は `GITLAB_ADMIN_PAT` 未設定の場合に docker exec 経由で管理者用 PAT を自動生成して System Hook を登録する
- `X-Idempotency-Key` ヘッダーによる重複受信抑止を実装する（同一キーの再送は処理せず 200 を返す）
- Webhook ペイロード不正時は WARNING ログを記録して 200 を返す（GitLab 側の再送機能に委譲）

### 3.16 LLMキー・モデル設定（F-6・F-9拡張）

ユーザー設定保存時にLLMキー妥当性チェックを行い、モデル候補をサジェスト表示する。

- Virtual Key 保存時に LiteLLM エンドポイントへキー妥当性確認リクエストを送信し、失敗時は保存を中止してGUIエラーを表示する
- デフォルトモデル入力フィールドにモデル候補を取得してサジェスト表示する（取得失敗時も手入力で保存継続可能）
...
| カテゴリ | 対応機能 |
| --- | --- |
| 業務機能 | F-1（System Hook受信）、F-2（ポーリング）、F-3（Issue→MR変換）、F-4（MR処理）、F-5（CLI実行環境管理）、F-8（進捗報告）、F-10（重複処理防止）、F-11（TTY入力待機検知）、F-12（System Hook 標準化）、F-13（LLMキー・モデル設定検証） |
