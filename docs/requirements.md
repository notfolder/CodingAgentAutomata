...
flowchart TD
    ...
    M --> N[Issueの処理中ラベルを削除しdoneラベルを付与・cli-execコンテナを破棄]
...
| F-10 | Web管理画面 | P-2 | ユーザー・Virtual Key設定・タスク実行履歴確認・プロンプトテンプレート管理をGUIで管理できない |
| F-11 | 重複処理防止 | P-1, P-3 | WebhookとポーリングがいずれもIssue/MRを同時検出した場合に同一タスクが2重に実行される |
...
| SC-07 | タスク実行履歴 | /tasks | 管理者（全ユーザー）、一般ユーザー（自分のデータのみ） |
| SC-08 | システム設定 | /settings | 管理者のみ |
...
- 表示項目: ユーザー名、メールアドレス、Virtual Key（マスク表示。末尾4文字のみ表示）、デフォルトCLI、デフォルトモデル、ロール、ステータス、登録日時、最終更新日時
...
- アクセス制御: 管理者のみアクセス可能
- 設定項目:
  - F-3用プロンプトテンプレート（テキストエリア。空欄の場合はシステムデフォルトを使用する）
  - F-4用プロンプトテンプレート（テキストエリア。空欄の場合はシステムデフォルトを使用する）
  - CLIアダプタ設定一覧（CLIエージェントIDごとにコンテナイメージ・起動コマンドテンプレート・情報渡し方のマッピングを管理する。組み込みアダプタの編集および新規アダプタの追加・削除が可能）
  - 進捗報告設定（MRへの進捗コメント投稿間隔（秒）・1コメントあたりの最大出力行数）
- 各テンプレートには使用可能な変数の一覧をヘルプテキストとして表示する
- 操作: 保存ボタン（成功時に同画面でフラッシュメッセージを表示）、リセットボタン（システムデフォルトに戻す。確認ダイアログ後に実行）
...
flowchart TD
    ...
    SC-03 -->|ナビゲーション| SC-07
    SC-07 -->|ナビゲーション: 管理者| SC-02
    SC-07 -->|ナビゲーション: 一般ユーザー| SC-03
    SC-02 -->|ナビゲーション: 管理者| SC-08[SC-08: システム設定]
    SC-08 -->|ナビゲーション| SC-02
```
...
Dockerデーモンを内包した本システム専用のカスタムイメージを使用する。Playwright・Python・npmなどメジャーな言語・テストフレームワークのテスト実行をサポートしたイメージを用意する。CLIエージェントごとに独立したイメージを維持する。
...
| フェーズ | 仕様 |
|---|---|
| 起動 | Consumerがタスク取得直後にcli-execコンテナを起動する。コンテナ名は `cli-exec-{cli_id}-{task_uuid}` とする。Virtual Keyを環境変数でセットする。リポジトリのgit cloneはConsumerがcli-execコンテナ内でPATを埋め込んだURL形式で実行し、GITLAB_PATはcli-execコンテナの環境変数には渡さない |
| タイムアウト | CLI処理開始から3時間を上限とし、超過した場合はCLIプロセスおよびコンテナを強制終了する |
| 正常完了後 | コンテナを即時破棄する |
| 異常終了・タイムアウト後 | コンテナを即時破棄する。Virtual Keyはコンテナ内の環境変数にのみ存在するため、コンテナ破棄によって確実に消去される |
...
| config_content_env | CLIが設定内容を環境変数経由で受け取る場合の、JSON/YAML形式の設定を渡す環境変数名（不要な場合は省略） |
...
| 起動コマンドテンプレート | `claude -p "{prompt}" --dangerously-skip-permissions --model {model}` | `opencode run "{prompt}" --model {provider}/{model}` |
| llm_api_key の渡し方 | 環境変数 `ANTHROPIC_API_KEY` | 環境変数 `ANTHROPIC_API_KEY`（Anthropicプロバイダー使用時） |
...
ConsumerはCLIアダプタ設定の `env_mappings` および `config_content_env` の定義に従って環境変数をセットする。コア処理はCLIエージェントの種別を直接参照しない。

cli-execコンテナ起動時にセットする環境変数はアダプタ設定によって決まるため、新CLIエージェントを追加する際にコア処理の変更は不要である。

**git認証（PAT埋め込みURL方式）**

cli-execコンテナには `GITLAB_PAT` を環境変数として渡さない。代わりに、ConsumerがCLI実行コンテナ内でリポジトリをgit cloneする際に、GITLAB_PATをURLに埋め込んだ形式（`プロトコル://oauth2:PAT@ホスト/プロジェクトパス.git`）を使用する。cloneされたリモートURLにPATが含まれるため、コンテナ内でのgit pushも追加の認証設定なしに実行できる。PATはコンテナの環境変数には存在しないため、コンテナ破棄後に残存しない。

**プロンプトテンプレート**

ConsumerはCLI起動時に渡す指示文をテンプレートから生成する。テンプレートはF-3・F-4で独立して管理し、システムデフォルトを提供する。ユーザー（管理者）はWeb管理画面のシステム設定でテンプレートを上書きできる。

| テンプレート種別 | 用途 | 埋め込む変数 |
|---|---|---|
| F-3用テンプレート | Issue→MR変換時のCLIへの指示文 | Issueタイトル・Issuedescription・プロジェクト名・リポジトリURL |
| F-4用テンプレート | MR処理時のCLIへの指示文 | MR description（そのまま展開）・ブランチ名・リポジトリURL |

デフォルトテンプレートでは、F-3はIssueの内容からブランチ名とMRタイトルの生成のみを行う最小限の指示とし、F-4はMR descriptionを指示の本体としてそのまま渡す構成とする。

MR descriptionにCLI種別・モデルの上書き指定が含まれる場合は、ユーザー設定のデフォルトより優先する（F-4のみ）。上書き指定の書式は本システムが定義したフォーマットに従う。
...
flowchart TD
    ...
    J --> J2[Issueの処理中ラベルを削除しdoneラベルを付与する]
    ...
    P --> P2[F-9 MRに処理開始コメントを投稿する]
    P2 --> Q[CLIがdescriptionの指示に従いコード変更・テストを実行する]
    Q --> Q2[F-9 CLIの標準出力を定期的にMRへ進捗コメントとして投稿する]
    Q2 --> R[変更をコミット・プッシュする]
    R --> S[F-9 MRの処理中ラベルを削除し完了コメントを投稿しdoneラベルを付与する]
```
...
| 業務 | 使用機能 |
|---|---|
| B-1（Issue→MR変換） | F-1, F-2（検出）/ F-3（変換処理）/ **F-5（cli-execコンテナ起動）**/ F-7（Virtual Key取得）/ F-9（進捗報告）/ **F-11（重複処理防止）** |
| B-2（MRコード実装） | F-1, F-2（検出）/ F-4（MR処理）/ F-5（cli-execコンテナ起動）/ F-7（Virtual Key取得）/ F-9（進捗報告）/ **F-11（重複処理防止）** |
...
いずれかのコンポーネントがダウンした場合はdocker-composeの再起動ポリシー（`restart: always`）により自動復旧する。
...
| 進捗コメント（初回作成） | CLI実行開始から最初の更新間隔（デフォルト60秒）経過時 | CLI標準出力の直近N行（デフォルト20行）を`<details>`で包んだ内容 |
| 進捗コメント（更新） | その後の更新間隔ごと | 同一コメントをGitLab APIで編集し、直近N行の内容に置き換える |
...
- summary行数（デフォルト20行）と更新間隔（デフォルト60秒）はシステム設定（SC-08）で変更可能とする
...
| 設定キー | デフォルト値 | 説明 |
|---|---|---|
| `progress_report_interval_sec` | `60` | 進捗コメントを更新する間隔（秒） |
| `progress_report_summary_lines` | `20` | `<summary>` に表示するCLI出力の末尾行数 |
| `progress_report_buffer_max_lines` | `20000` | メモリ内に保持するCLI出力の最大行数。超過時は古い行から破棄する |

### 3.13 重複処理防止（F-11）

Webhookとポーリングは独立したプロセスとして同時動作するため、同一Issue/MRを両方が検出した場合に同じタスクが2重に実行される可能性がある。以下の仕組みで重複を防止する。

**方針: DB部分ユニーク制約 + INSERT ON CONFLICT DO NOTHING**

- ProducerはタスクをキューへINSERTする前に、まずPostgreSQLの `tasks` テーブルへタスクレコードをINSERTする
- `tasks` テーブルの `(gitlab_project_id, source_iid, task_type)` の組み合わせに対して、`status IN ('pending', 'running')` の行が存在する場合に一意となる**部分ユニーク制約**を設ける
  - 制約名: `uq_tasks_active` （`status` が `pending` または `running` の行のみ対象）
- INSERT成功時のみRabbitMQキューへタスクを投入する
- INSERT失敗（制約違反）時はキューへの投入をスキップする
- `status` が `completed` または `failed` になった後は制約対象外となるため、同一Issue/MRに再度ラベルが付与された場合は新規タスクとして処理できる

**制約の適用範囲**

| 制約対象 | 制約外 |
|---|---|
| `status = 'pending'` または `status = 'running'` の行 | `status = 'completed'` または `status = 'failed'` の行 |
...
> **部分ユニーク制約（F-11）**: `(gitlab_project_id, source_iid, task_type)` の組み合わせは、`status IN ('pending', 'running')` の行の中で一意でなければならない。完了・失敗済みのレコードは制約対象外。
...
| config_content_env | CLIが設定内容を環境変数経由でJSON/YAML形式で受け取る場合の環境変数名。不要な場合はNULL | |
...
| GitLab PAT | 環境変数のみで管理し、DBには保存しない。cli-execコンテナには環境変数として渡さず、git clone URLにPATを埋め込んだ形式で使用する。コンテナ破棄後にPATが残存しない |
...
| サービス | 提供方法 |
|---|---|
| GitLab | `docker-compose` に GitLab Community Edition のコンテナを含め、本システムと同一 compose 環境で起動する |
| LiteLLM Proxy | `docker-compose` に LiteLLM Proxy コンテナを含め、同一 compose 環境で起動する。`OPENAI_API_KEY` を `.env` から受け取り、OpenAI モデルへの転送設定を初期状態で含む |

#### `.env` の必須定義項目（テスト実行最小構成）

| 変数名 | 内容 |
|---|---|
| `OPENAI_API_KEY` | OpenAI の APIキー。LiteLLM Proxy がモデルへの転送に使用する |

上記1項目を `.env` に設定した状態で `docker-compose up` を実行すると、GitLab・LiteLLM Proxy・本システム全コンポーネントが起動し、§6 のe2eテストシナリオを実施できる状態になることを設計上の制約とする。GitLab の初期設定（botアカウント作成・PAT発行・Webhook設定・テストプロジェクト作成）は `docker-compose up` 後に実行するセットアップスクリプトで自動化する。
...
| コンテナイメージ | `claude` アダプタ用・`opencode` アダプタ用それぞれの `container_image` に対応するDockerイメージがDocker Hubまたはローカルレジストリからpull可能であること |
| テストユーザー | `default_cli = claude` に設定したユーザーAと `default_cli = opencode` に設定したユーザーBをDBに登録済みであること。§5.4 のセットアップスクリプトで自動作成される |
| Virtual Key | ユーザーA・ユーザーB それぞれに LiteLLM Proxy への有効なVirtual Keyが設定されていること。セットアップスクリプトが LiteLLM Proxy API 経由で自動発行・登録する |
| LiteLLMモデル設定 | `claude` 用モデル（例：`claude-opus-4-5`）および `opencode` 用モデル（例：`openai/gpt-4o`）への転送設定が LiteLLM Proxy に存在すること。`openai/gpt-4o` は `OPENAI_API_KEY` のみで動作し、初期設定に含まれる |
| GitLabアカウント | 上記ユーザーA・ユーザーBのメールアドレスと一致するGitLabアカウントが存在し、テスト用プロジェクトへのアクセス権を持つこと。セットアップスクリプトが GitLab API 経由で自動作成する |
| Webhook設定 | テスト用プロジェクトに本システムのWebhookサーバーURLが登録されていること。セットアップスクリプトが自動設定する |

CLIアダプタごとに上記前提条件を満たした状態でシナリオ一覧のT-01〜T-30を順に実施し、両アダプタで同一の期待結果が得られることを確認する。T-08（CLI上書き指定）は2種のアダプタが共存していることを前提とするため、切り替え先アダプタが有効な状態であることを確認してから実施する。
...
| T-08 | MR descriptionのCLI指定でデフォルトを上書きできる | T-01完了（デフォルトCLIが特定のcli_id A）、MRにbotアサイン・特定ラベルあり | MR descriptionの先頭CLI種別（異なるcli_id B）とモデルの上書き指定を記述する。Webhook/ポーリングを待つ | ユーザーのデフォルトCLI（cli_id A）を無視して、descriptionで指定したCLI・モデルで処理が実行される |
...
| T-18 | cli-execコンテナでdocker-composeによるe2eテストが実行できる | T-07と同様の前提 | MR descriptionにdocker-composeを使ったe2eテスト実行を指示として記述する | CLIがdocker-composeでサービスを起動し、e2eテストを実行した結果がコミットに含まれ、MRにプッシュされる |
| T-19 | 一般ユーザーが自分のユーザー詳細を閲覧できる | T-01完了、一般ユーザーでSC-01からログイン済み | SC-03の自分のユーザー詳細ページを開く | 自分のユーザー名・メールアドレス・デフォルトCLI等の情報が表示される |
| T-20 | 一般ユーザーが自分のメールアドレス・デフォルトCLI・モデルを編集できる | T-01完了、一般ユーザーでSC-01からログイン済み | SC-05（自分の編集画面）でメールアドレス・デフォルトCLI・デフォルトモデルを変更して「保存」を押す | 変更がDBに反映される。Virtual Key・ロール・ステータスの変更項目は画面に表示されない |
| T-21 | 一般ユーザーが他ユーザーの詳細を閲覧できない | T-01完了、一般ユーザーでSC-01からログイン済み | 一般ユーザーが他ユーザーのSC-03 URL（/users/other_user）を直接開く | 403エラーが返る |
| T-22 | 一般ユーザーが他ユーザーを編集できない | T-01完了、一般ユーザーでSC-01からログイン済み | 一般ユーザーが他ユーザーのSC-05 URL（/users/other_user/edit）を直接開く | 403エラーが返る |
| T-23 | 一般ユーザーがSC-02（ユーザー一覧）にアクセスできない | T-01完了、一般ユーザーでSC-01からログイン済み | 一般ユーザーがSC-02 URL（/users）を直接開く | 403エラーが返る |
| T-24 | 同一メールアドレスでユーザーを登録できない | T-01完了（登録済みユーザーのメールアドレス既知） | SC-04で既存ユーザーと同じメールアドレスを入力し「作成」を押す | メールアドレス重複エラーが表示され、ユーザーが登録されない |
| T-26 | 一般ユーザーが自分のタスク実行履歴を閲覧できる | T-07完了、一般ユーザーでSC-01からログイン済み | SC-07（タスク実行履歴）を開く | 自分のタスクのみが表示される。ユーザー名フィルタは自分のみ固定で変更できない |
| T-27 | WebhookとポーリングがIssue/MRを同時検出しても重複処理されない | T-01完了、WebhookとポーリングをともにEnabledにして同時動作させた状態でGitLabにIssueが存在する | Issueにbotアサイン＋特定ラベルを付与してWebhookイベントとポーリングが同時に反応するよう操作する | タスクが1件のみ作成され、Issue→MR変換処理が1回のみ実行される。2重にDraft MRが作成されない |
| T-28 | 管理者がSC-08でF-3用プロンプトテンプレートを変更できる | T-01完了（管理者でログイン済み） | SC-08（システム設定）を開き、F-3用テンプレートを編集して保存する | 保存成功のフラッシュメッセージが表示され、次回のF-3処理で変更後のテンプレートが使用される |
| T-29 | 管理者がSC-08でF-3用プロンプトテンプレートをリセットできる | T-28完了（テンプレートをカスタマイズ済み） | SC-08でF-3用テンプレートの「リセット」ボタンを押し確認ダイアログで実行する | テンプレートがシステムデフォルトに戻り、次回のF-3処理でデフォルトテンプレートが使用される |
| T-30 | 一般ユーザーがSC-08にアクセスできない | T-01完了（一般ユーザーでログイン済み） | /settings のURLへ直接アクセスする | 403エラーが返却される |
| T-31 | MR処理中にCLI出力が1つの進捗コメントに定期更新される | T-01完了、処理時間が60秒以上かかるMRが存在 | T-07と同様の手順でMR処理を開始し、処理中のMRコメントを60秒ごとに確認する | 進捗コメントが1件作成され、60秒ごとにCLIの直近20行出力（`<details>`で包まれた形式）に上書き更新される。処理完了後に完了コメントが別途投稿される |
| T-32 | 管理者がSC-08で進捗コメント更新間隔と表示行数を変更できる | T-01完了（管理者でログイン済み） | SC-08の進捗報告設定で更新間隔を120秒・表示行数を30行に変更して保存する | 保存成功のフラッシュメッセージが表示され、次回のMR処理から120秒間隔・直近30行で進捗コメントが更新される |
...
| カテゴリ | 対応機能 |
|---|---|
| 業務機能 | F-1（Webhook受信）、F-2（ポーリング）、F-3（Issue→MR変換）、F-4（MR処理）、F-5（CLI実行環境管理）、F-9（進捗報告）、F-11（重複処理防止） |
| マスタ管理 | F-6（ユーザーCRUD管理）、F-7（Virtual Key暗号化管理）、CLIアダプタ設定CRUD管理（SC-08） |
...
| 運用 | F-10（Web管理画面：ユーザー管理・タスク履歴・プロンプトテンプレート管理・CLIアダプタ設定管理） |
...
stateDiagram-v2
    ...
    running --> failed : CLIエラー / cli-execコンテナ起動失敗 / ユーザー未登録 / 無効ユーザー
```
...
| 管理者による追加 | SC-08にて新規CLIアダプタを登録できる |
| 管理者による編集 | SC-08にて既存アダプタの各属性を変更できる |
...
| 要件 | 削除の可否 | 判定理由 |
|---|---|---|
| SC-02のダッシュボード表示 | **削除済み** | SC-07（タスク履歴）で業務課題P-1に対応でき、ログイン後の遷移先をSC-02（ユーザー一覧）に変更することでMVPを達成できる |
| SC-07のタスク詳細表示（モーダル等） | **削除済み** | タスク一覧の表示項目で十分。詳細ドリルダウンはMVPに不要 |
| CLIログの全件保存 | **維持** | エラー発生時の調査に必要。削除するとシステム障害時の原因特定ができなくなる |
| 処理中ラベルの管理 | **維持** | GitLab上で処理状態を視覚的に確認するために必要。二重処理防止はF-11（DB部分ユニーク制約）が担う |
