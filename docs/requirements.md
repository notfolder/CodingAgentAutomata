...
- アクセス制御: 一般ユーザーのタスク取得は常に自分のユーザー名に固定する（他ユーザー指定クエリは無視して自分のデータのみ返す）
- ページネーション: あり（1ページ20件）
...
| T-04 | IssueをWebhookで検出してMR変換できる | T-01完了、GitLabにIssue存在、Webhook設定済み | GitLabでIssueにbotをアサイン＋特定ラベルを付与する | Webhookイベント受信後、Issue→MR変換処理が実行される。GitLabに作業ブランチとDraft MRが作成される。IssueにMR作成完了コメントが投稿される。Issueはクローズされない |
...
| T-19 | 一般ユーザーが他ユーザーの詳細を閲覧できない | T-01完了、一般ユーザーでSC-01からログイン済み | 一般ユーザーが他ユーザーのSC-03 URL（/users/other_user）を直接開く | SC-06（/tasks）へリダイレクトされる |
| T-20 | 一般ユーザーが他ユーザーを編集できない | T-01完了、一般ユーザーでSC-01からログイン済み | 一般ユーザーが他ユーザーのSC-05 URL（/users/other_user/edit）を直接開く | SC-06（/tasks）へリダイレクトされる |
