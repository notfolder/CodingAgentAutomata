...
| F-12 | System Hook 標準化 | P-1, P-3 | GitLab CE では Group Webhook が利用できないため、GitLab CE でも動作するインスタンス全体向けの System Hook を標準方式とする |
...
GitLab System Hook をイベント受信の標準方式とする。

- GitLab 管理者がインスタンス全体に対して System Hook を一度設定することで全プロジェクトのイベントを受信する
...
| カテゴリ | 対応機能 |
| --- | --- |
| 業務機能 | F-1（Webhook受信）、F-2（ポーリング）、F-3（Issue→MR変換）、F-4（MR処理）、F-5（CLI実行環境管理）、F-8（進捗報告）、F-10（重複処理防止）、F-11（TTY入力待機検知）、F-12（System Hook 標準化）、F-13（LLMキー・モデル設定検証） |
