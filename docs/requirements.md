...
| F-12 | Group Webhook標準化 | P-1, P-3 | プロジェクトごとにWebhookを設定する運用負荷が高く、設定漏れが発生しうる |
...
### 3.15 Group Webhook運用（F-1拡張）

GitLab Group Webhook を受信の標準方式とする。

- グループ配下の全プロジェクトのイベントを単一 Webhook エンドポイントで受信する
...
| カテゴリ | 対応機能 |
| --- | --- |
| 業務機能 | F-1（Webhook受信）、F-2（ポーリング）、F-3（Issue→MR変換）、F-4（MR処理）、F-5（CLI実行環境管理）、F-8（進捗報告）、F-10（重複処理防止）、F-11（TTY入力待機検知）、F-12（Group Webhook標準化）、F-13（LLMキー・モデル設定検証） |
