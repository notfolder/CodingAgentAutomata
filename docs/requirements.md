...
- 変更可能項目（管理者が全ユーザーを編集する場合）:
  ...
  - ユーザー個別F-4テンプレート（テキストエリア。設定するとSC-07のシステム設定より優先される。使用可能な変数の一覧をヘルプテキストとして表示する）
...
- アクセス制御: 管理者のみアクセス可能
- 設定項目:
  - F-3用プロンプトテンプレート（テキストエリア。初期値はシステムセットアップスクリプトでDB投入済み。空欄の場合はプロンプトなし）
  - F-4用プロンプトテンプレート（テキストエリア。初期値はシステムセットアップスクリプトでDB投入済み。空欄の場合はプロンプトなし）
  - CLIアダプタ設定一覧（CLIエージェントIDごとにコンテナイメージ・起動コマンドテンプレート・情報渡し方のマッピングを管理する。組み込みアダプタは編集のみ可能で削除不可。新規アダプタの追加・編集・削除が可能）
    - **CLIアダプタ追加・編集フォームの入力項目**:
      ...
      - MCP設定環境変数名（省略可。CLIにMCP設定を渡す環境変数名）
...
- 操作: 保存ボタン（プロンプトテンプレート・システムMCP設定・進捗報告設定を一括保存する。成功時に同画面でフラッシュメッセージを表示。CLIアダプタ設定は個別の追加・編集フォームで保存する）、各テンプレートにはクリアボタン（テンプレートを空欄（プロンプトなし）にする。確認ダイアログ後に実行）、進捗報告設定にはリセットボタン（初期値に戻す。確認ダイアログ後に実行）
...
| 起動コマンドテンプレート | `claude -p "{prompt}" --dangerously-skip-permissions --model {model}` | `opencode run "{prompt}" --model {model}` |
...
| mcp_config_env | `CLAUDE_MCP_CONFIG` | `OPENCODE_MCP_CONFIG` |
...
ConsumerはCLI起動時に渡す指示文をテンプレートから生成する。テンプレートはF-3・F-4で独立してDBに保存し、初期値はシステムセットアップスクリプトで投入する。管理者はSC-07（システム設定）でシステムテンプレートを変更できる。F-4については一般ユーザーもSC-05（ユーザー編集）でユーザー個別テンプレートを設定でき、システムテンプレートより優先される。F-4用テンプレートが空欄の場合はプロンプトなしとなり、CLIへの指示文は送られない。F-3用テンプレートは管理者のみが変更可能であり、空欄にした場合はF-3処理が失敗するため管理者は常に設定を維持すること。
...
MR descriptionにCLI種別・モデルの上書き指定が含まれる場合は、ユーザー設定のデフォルトより優先する（F-4のみ）。上書き指定はMR descriptionの先頭行にHTMLコメント形式で記述し、コメント内に本システム固有の識別子に続けて `cli=<CLIエージェントID>` および `model=<モデル名>` をスペース区切りで指定する。各キーは省略可能で、指定されたキーのみが上書き対象となる。HTMLコメントはGitLabのMarkdownレンダリング時に非表示となるため、descriptionの可読性を損なわない。
...
- summary行数（デフォルト20行）と更新間隔（デフォルト60秒）はシステム設定（SC-07）で変更可能とする
...
| mcp_config_env | CLIにMCP設定（システム設定とユーザー設定をマージしたJSON）を渡す環境変数名。不要な場合はNULL | |
...
- **GitLabセットアップ**: GitLab API経由でbotアカウント作成・PAT発行を行い、対象プロジェクトへのWebhook設定を登録する
...
- **GitLabセットアップ**: GitLab API経由でテスト用プロジェクト作成・テストユーザーアカウント作成・テストプロジェクトへのWebhook設定を行う
...
| T-26 | 管理者がSC-07でF-3用プロンプトテンプレートをクリアできる | T-25完了（テンプレートをカスタマイズ済み） | SC-07でF-3用テンプレートの「クリア」ボタンを押し確認ダイアログで実行する | テンプレートが空欄になり、次回のF-3処理ではプロンプトなしで実行される |
...
| T-29 | 管理者がSC-07で進捗コメント更新間隔と表示行数を変更できる | T-01完了（管理者でログイン済み） | SC-07の進捗報告設定で更新間隔を120秒・表示行数を30行に変更して保存する | 保存成功のフラッシュメッセージが表示され、次回のMR処理から120秒間隔・直近30行で進捗コメントが更新される |
