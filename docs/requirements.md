...
- 操作: 保存ボタン（成功時に同画面でフラッシュメッセージを表示）、各テンプレートにはクリアボタン（テンプレートを空欄（プロンプトなし）にする。確認ダイアログ後に実行）、進捗報告設定にはリセットボタン（初期値に戻す。確認ダイアログ後に実行）
...
| botアサイン解除 | ConsumerはF-4処理中、進捗更新と同じ間隔でMRのレビュアー一覧をGitLab APIで確認する。botアカウントがレビュアーから外された場合はCLIプロセスおよびコンテナを強制終了する。その後 git push を実行し（エラーの場合も無視）、コンテナを即時破棄する。MRに強制終了コメントを投稿する（F-3には適用しない） |
| 正常完了後 | コンテナを即時破棄する |
...
| 強制終了コメント | botアカウントがMRのレビュアーから外されたことを検知した時（F-4のみ） | botアサイン解除を機にCLIを強制終了した旨・git push結果（成功/失敗）・タスクUUID |
