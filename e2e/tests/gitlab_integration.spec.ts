...
const ISSUE_FILE_TASK_DESCRIPTION =
  '固定文字列「E2E_FIXED_TEXT」を内容に含む e2e-fixed.txt を作成する作業用のブランチ名と Draft MR タイトルを提案してください。';

const MR_FILE_TASK_DESCRIPTION =
  '固定文字列「E2E_FIXED_TEXT」を内容に含む e2e-fixed.txt を作成し、コミットしてプッシュしてください。';
...
/**
 * 不安定になりやすいテストの直前に実行するバリア。
 * admin でログインし、キューが掃けるまで待機する。
 */
async function waitForFlakyTestBarrier(): Promise<void> {
...
test('T-06: MR変換後にbotアサイン・ラベルが引き継がれる', async () => {
  ...
  // Issue 作成（testuser-claude として作成することで backend の users テーブルに存在するユーザーが author になる）
  const issue = await createIssue(projectId, GITLAB_USER_TOKEN, issueTitle, ISSUE_FILE_TASK_DESCRIPTION);
  // bot アサイン + ラベル付与—admin 権限で実施
  await triggerIssue(projectId, issue.iid, GITLAB_ADMIN_TOKEN, GITLAB_BOT_NAME, GITLAB_BOT_LABEL);
...
test('T-10: 未登録ユーザーのIssueは処理されない', async () => {
  ...
  // 存在しないユーザー名でアサインを試みる
  const issue = await createIssue(projectId, GITLAB_ADMIN_TOKEN, issueTitle, ISSUE_FILE_TASK_DESCRIPTION);
...
test('T-13: 無効化ユーザーのIssueは処理されない', async () => {
  ...
  const issue = await createIssue(projectId, GITLAB_ADMIN_TOKEN,
    `[E2E T-13] 無効化ユーザーテスト ${issueSuffix}`, ISSUE_FILE_TASK_DESCRIPTION);
...
test('T-28: MR処理中にCLI出力が進捗コメントに定期更新される', async () => {
  ...
  await gitlabApi('PUT', `/projects/${projectId}/merge_requests/${mr!.iid}`, GITLAB_ADMIN_TOKEN, {
    description: MR_FILE_TASK_DESCRIPTION,
  });
