...
/**
 * ログイン操作を行う共通ヘルパー
 * autocomplete 属性で直接 input 要素を特定する
 */
...
/**
 * 一般ユーザーとしてログインして /tasks への遷移を待つヘルパー
 */
async function loginAsUser(page: import('@playwright/test').Page, username: string): Promise<void> {
  await login(page, username, process.env.TEST_USER_PASSWORD ?? 'Test@123456');
...
test('T-19: 一般ユーザーが他ユーザーの詳細ページにアクセスできない', async ({ page }) => {
  ...
  // 他ユーザーの詳細ページへ直接遷移後、Vue Router ガードによるリダイレクトを待つ
  await page.goto('/users/admin');
  await page.waitForURL(url => !url.toString().includes('/users/admin'), { timeout: 5000 }).catch(() => {});
...
test('T-20: 一般ユーザーが他ユーザーの編集ページにアクセスできない', async ({ page }) => {
  await loginAsUser(page, 'testuser-opencode');
  // 他ユーザーの編集ページへ直接遷移後、Vue Router ガードによるリダイレクトを待つ
  await page.goto('/users/admin/edit');
  await page.waitForURL(url => !url.toString().includes('/users/admin/edit'), { timeout: 5000 }).catch(() => {});
