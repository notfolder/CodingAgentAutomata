...
class RabbitMQClient:
    ...
    def consume(self, callback: Callable[[dict], None]) -> None:
        ...
        def _schedule_stop_after_current_task() -> None:
            """処理中タスク完了後に consume ループを停止する。"""
        ...
        def _finalize_message(ch, delivery_tag: int, succeeded: bool) -> None:
            """consume スレッドで ACK/NACK と停止判定を行う。"""
            try:
                if ch.is_open:
                    if succeeded:
                ...
                logger.warning("RabbitMQ ACK/NACK エラー（無視）: %s", exc)
            finally:
                self._active_worker = None
                self._worker_done.set()
                if self._stop_requested:
                    _schedule_stop_after_current_task()

        def _run_callback_in_worker(ch, delivery_tag: int, message: dict) -> None:
            """長時間タスクを別スレッドで実行し、完了後に ACK/NACK を依頼する。"""
            succeeded = False
            try:
                callback(message)
                succeeded = True
                ...
                if self._connection and self._connection.is_open:
                    self._connection.add_callback_threadsafe(
                        lambda: _finalize_message(ch, delivery_tag, succeeded)
                    )
                else:
                    self._active_worker = None
                    self._worker_done.set()
        ...
        def _on_message(ch, method, properties, body):
            """pika コールバック。JSONデコード後に別スレッドで task を実行する。"""
            try:
                message = json.loads(body.decode("utf-8"))
                logger.debug(
                    "RabbitMQ received: queue=%s, message=%s",
                    self._queue_name,
                    message,
                )
            except Exception as exc:
                logger.error("RabbitMQ message decode error: %s", exc, exc_info=True)
                ch.basic_nack(delivery_tag=method.delivery_tag, requeue=True)
                return

            self._worker_done.clear()
            worker = threading.Thread(
                target=_run_callback_in_worker,
                args=(ch, method.delivery_tag, message),
                name=f"rabbitmq-task-{self._queue_name}",
                daemon=True,
            )
    ...
    def stop_consuming(self) -> None:
        ...
        try:
            ...
            if self._connection and self._connection.is_open and self._channel:
                ...
                def _stop() -> None:
                    """処理中タスクがなければ即停止し、あれば完了後停止に切り替える。"""
                    try:
                        if self._active_worker is None:
                            ...
                            logger.info(
                                "RabbitMQ stop_consuming を保留しました: 処理中タスクの完了を待ちます"
                            )
                    except Exception as exc:
                        logger.warning("RabbitMQ stop_consuming 内部エラー（無視）: %s", exc)
